 ...
 */
	...
	public function services_notification() {
	
		if ($this -> request -> is('get')) {
			
			$result 	= $this->Notification->find('first', array('recursive' => -1, 'order' => array('Notification.created DESC'),'conditions'=> array('Notification.driver_id'=>$this->request->data[0]['driverId'], 'Notification.created >' =>'BETWEEN NOW() AND NOW()- INTERVAL 1 minutes')  )));
			
			$data = array('success' => true, 'message' => "Job Posted successfully", 'data' => $result);
		
		} else {
			$data = array('success' => false, 'message' => "Google", 'data' => null);
		}
		$this -> set('data', $data);
	}
